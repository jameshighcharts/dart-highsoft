...
The verified Google email must exactly match the email returned by Slack, ignoring case and surrounding whitespace. Mapping uses that email and the returned user ID; optional workspace and membership-status fields do not block a match. The bot token determines which Slack workspace is searched. Email aliases and names are not used as fallbacks. If the emails differ, sign in with Slack or correct the workspace directory email through its administrator. Slack returns `users_not_found` for deactivated accounts.
