...
Run `installHighdartsSync` once in the bound script to refresh and create one five-minute trigger owned by the installing Google account. Repeated installation does not duplicate the trigger. `@OnlyCurrentDoc` limits spreadsheet authorization to this workbook. A Highdarts menu also provides manual refresh. The script verifies all 76 office/fixture-number pairs, table sizes and the entire response before editing any cells. A fetch failure or invalid response leaves existing values intact. An execution error is visible in Apps Script Executions. The published iframe may lag the editable sheet by several minutes. The Bengt Sheet tab requests a fresh iframe URL when opened, every minute while visible, when returning to the browser tab, and through its Refresh sheet button. Closing the Sheet tab stops its timer; refreshing the iframe does not run the Apps Script sync or bypass Google publication delays.
...
## Manual tournament scoring

An unplayed head-to-head fixture can start with **Board → Manual scoring**. Open its start link from Bengt, or select its two players on New Game and enable Highdarts 2026. The fixture, round settings and first-to-two format remain enforced by the database. Already-started fixtures cannot be started again. Scolia readiness is required only when selecting Scolia; a known busy office or active tournament player still blocks the start.

Manual mode leaves `scolia_board_id` empty, so hardware throws cannot enter the match. The office remains reserved until the match finishes or ends early. The shared database board lock prevents a Scolia friendly or party game from starting after the manual tournament match; the reverse ordering is also rejected. The board picker displays this reservation. An explicit manual choice survives board refreshes and skips the ordinary suggestion to use an available Scolia board.

Deploy `20260915120000_highdarts_manual_scoring.sql` before deploying this change. It replaces two existing guard functions, adds pause status to the snapshot and preserves their privileges; it adds no columns and changes no stored results. This migration was applied to production project `imezdipfchojmpobario` through the Supabase SQL editor on September 15, 2026. Its full name and exact source are recorded atomically in `supabase_migrations.schema_migrations`; the stored source matches repository MD5 `92cf2bc9188f56444c395e869c1db67d`. The previous counted-results migration is also in production.

Verification: run `supabase/tests/highdarts_manual_scoring.sql` in a disposable database and `HIGHDARTS_MANUAL_NATIVE=1 node scripts/highdarts-manual.integration.mjs` with the prepared local API on port 3022. Roll back the app first if needed. Keep the new board guard while any manual tournament match remains active; after those finish, the guard definitions from the earlier board migrations can be restored to disable future manual starts.

The Highdarts header shows a live game count beside Today, with compact pairing, location and spectator links inside the Live stat. Paused matches retain their label and are excluded from the Live count. No status section appears when there are no ongoing matches. Names link to profiles, active statuses open spectator view, and finished games remain in Recent results. Early-ended games do not count as played. Status refreshes with the existing snapshot every 15 seconds and on window focus.

Validation on September 15: all 1,486 unit/component tests passed; lint had zero errors and 23 existing warnings; the webpack production build passed. Both database regressions passed, including snapshot pause/early-end transitions. The real API concurrency check returned one successful start and one conflict with no orphan match. Browser checks confirmed manual selection survives board refresh, a manual dart persists from 301 to 281, live/paused header rows link to the correct players and match, and reported results show their leg scores. Browser test matches were removed from the isolated database. The local REST preview does not provide Supabase Realtime or real Scolia hardware, so those transports were not exercised.

Production verification on September 15: all three installed function definitions match the tested local definitions, with their existing execute privileges and security modes unchanged. All 76 fixtures remain. The snapshot succeeds under `service_role` and includes pause status on linked matches. Production verification was read-only; no test matches or fixture changes were committed. Manual creation, board contention and duplicate-claim behavior were exercised in the isolated database and API checks above.

## Ending and restarting a Bengt fixture

Deploy `20260917150000_reset_ended_highdarts_matches.sql` before the updated application. Ending a Bengt match calls the service-only `end_match_early_atomic` RPC. It locks the match, refuses completed results, removes the abandoned attempt and its dependent scoring data, clears the fixture link and queued result job, and returns the scorer to Bengt. The original fixture, players, counting choices, and bracket links stay intact, and the next start can select its board again. The confirmation explicitly states that the attempt's scores are discarded. Ordinary matches retain their existing ended-early history.

Legacy ended-early attempts without a published result can also be reset through this endpoint. An active result delivery or locked group draw rejects the entire transaction. No historical attempts are changed by deploying the migration alone.

Rollback: deploy the previous application first, then drop `public.end_match_early_atomic(uuid)` if desired. Already-reset attempts cannot be reconstructed by rolling back code; restore them only from a database backup. Run `supabase/tests/end_match_early.sql` against a disposable migrated database to verify the database behavior.
