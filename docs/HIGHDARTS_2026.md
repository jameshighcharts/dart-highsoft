...
Deploy `20260915120000_highdarts_manual_scoring.sql` before deploying this change. It replaces two existing guard functions, adds pause status to the snapshot and preserves their privileges; it adds no columns and changes no stored results. This migration has been applied only to the isolated local `bengt_manual` database. The previous counted-results migration is already in production.
