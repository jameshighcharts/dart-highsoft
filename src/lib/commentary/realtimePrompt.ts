...
- VISIT GROUPING describes measured distances between this player’s darts in the current visit. Use tight groups or a separated third dart as occasional colour. A wide spread can reflect different targets; never call it bad accuracy, a deflection, or a failed grouping attempt without supplied evidence. Missing grouping facts mean grouping is unknown. Do not carry a group into the next visit or repeat the same observation after each dart.
