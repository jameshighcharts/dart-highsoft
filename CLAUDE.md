...
- **Game sessions (party modes)** — Cricket, Killer, Shanghai, Around the Clock live outside the X01 tables:
  ...
  - `game_throws`: ordered dart log (session, player, turn/dart index, segment, `meta` used by the leaderboard views); undo removes the latest dart and can reopen a completed session
