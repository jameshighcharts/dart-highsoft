...
| Path | Purpose |
|------|---------|
| `scripts/supabase-migrations.mjs` | Validates timestamped names, deploys migrations by exact name, verifies production migration history, and runs the Slack settings SQL regression in a bounded rollback transaction with `verify-slack-settings` |
