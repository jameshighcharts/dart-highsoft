...
- `supabase/migrations/20260909170000_persist_prepare_scolia_throw.sql`: Service-only combined event persistence and scoring preparation; preserves duplicate payloads and raw-event recovery on preparation failures. Normal X01 scoring uses two sequential database requests; board status remains a separate overlapping write. The local atomic integration harness asserts the two-request path across 420 darts.
