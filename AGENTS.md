...
- `src/lib/slack/members.test.ts`: Covers exact normalized email matching without membership-metadata requirements, mismatched emails, malformed responses, configuration failures, and cached lookup recovery.
...
| `server/scoliaThrowIngestion.ts` | Idempotently maps persisted Scolia detections into the active app match and completes turns/legs; compact count/total snapshots select throwers while fresh raw-dart snapshots preserve correction-safe settlement |
...
| `highdarts/Dashboard.tsx` | Oslo-day progress with a hover/click Today summary of completed tournament games, leg scores and labeled averages, office cards, searchable six-fixture pagination, recent-result scorecards with player avatars, winner highlights, aligned averages and expandable reported legs, compact side-by-side Tabell tables with one status pill per player, Sluttspill and dark-filtered Sheet tabs |
...
| `games/BoardPicker.tsx` | New-game board dropdown, live status labels, and location-to-board matching; offline boards remain selectable so the creator's local board stays selected until they explicitly choose Manual scoring; `BoardPicker.test.ts` covers matching when a board is offline or absent |
...
| `CompassTvButton.tsx` | Direct link to the Compass TV slideshow in the same tab after X01 matches and party games, in scorer and spectator views |
...
| `GridLeaderboard.tsx` | Home page leaderboard grid with dark slate table and KPI backgrounds matching game settings, Elo filters on the left and location filters on the right of the desktop toolbar, and stacked mobile filters; players and their Elo appear after 3 completed X01 matches, excluding early endings |
...
**Remake match:** Active or completed standalone X01 matches expose Remake match in the bull-off, scoring, and spectator views. `MatchClient` copies the current rules and ordered player IDs into `NewGameOptions` saved setup, then opens `/new?remake=1` for review before a new match is created. Tournament matches do not expose this action.
...
### Scolia latency implementation
- `src/lib/server/scoliaAtomicIngestion.ts`: Existing TypeScript scoring rules produce a revision-checked plan; the worker caches raw state only while the database source revision matches. Atomic RPC commits the dart, settlement, durable analytics jobs, and processed event together. Bull-off, party games, and legacy partial events retain their existing ingestion paths.
- `supabase/migrations/20260909150000_compact_scolia_selection.sql`: Compact legacy turn-selection RPC; corresponding SQL regression fixture is in `supabase/tests`.
- `supabase/migrations/20260909160000_atomic_scolia_scoring.sql`: Service-only prepare/commit, accepted joined facts, and revision-checked edit/undo recomputation RPCs; corresponding SQL regression fixture is in `supabase/tests`.
- `scripts/scoliaAtomic.integration.mjs`: Local-only, rolled-back real SQL/TypeScript integration harness for long seven-player games, duplicates, undo, correction races, fair ending, leg rotation, and Elo.
- `src/services/scoliaCommentaryAnalysis.ts` and `src/workers/scoliaCommentaryAnalysisWorker.ts`: Isolate commentary replay/models/snapshots from scoring's event loop; bounded requests, correction invalidation, timeouts, and crash recovery. Service tests and `scripts/workerLoader.test.mjs` cover lifecycle and actual thread startup.
- `src/lib/commentary/realtimeSessionCache.ts`: Bounded listener cache with notification invalidation, heartbeat/session expiry, and in-flight fetch protection; colocated regression tests.
- `src/services/scoliaRealtimeCommentaryPublisher.ts`: Uses accepted committed facts and the analysis thread, invalidates stale listener/correction work, preserves response-level persona instructions, and rehydrates current facts under bounded conversation retention.
- `src/lib/server/scoliaAtomicIngestion.test.ts`: Pure planning parity for normal, bust, correction, and repeated fair-ending tiebreak behavior.

- `supabase/migrations/20260909170000_persist_prepare_scolia_throw.sql`: Service-only combined event persistence and scoring preparation; preserves duplicate payloads and raw-event recovery on preparation failures. Provides the cold/recovery path; speculative scoring uses the later migration.

- `supabase/migrations/20260909180000_speculative_scolia_commit.sql`: Service-only one-request warm scoring, revision rejection with fresh-state fallback, and durable raw-event recovery on failed speculative commits; exercised by the local atomic integration harness.
- `supabase/migrations/20260909181000_live_scoring_broadcast.sql`: Database-stamped live row versions and private broadcast receive authorization following match-read RLS; browser publishing remains denied. Local SQL integration checks both anon/authenticated permissions.
- `src/lib/match/liveScoringBroadcast.ts` and its tests: Validated turn/dart broadcast contract and per-row ordering across broadcasts, WAL echoes, edits, deletes, and HTTP snapshots.
- `src/services/scoliaScoreBroadcaster.ts` and its tests: Best-effort private committed-score broadcasts; uses HTTP while joining then the persistent socket, with bounded in-flight sends and WAL fallback.
- `src/lib/scolia/boardStatusWriter.ts` and its tests: Separate ordered status retry queue with unchanged-state coalescing and five-second active heartbeat refresh; scoring never waits for board-status persistence.
- `src/hooks/useMatchRealtime.ts`: Scoring view uses the shared payload reducers and preserves optimistic local darts; private committed-score broadcasts populate parent turns before darts, reject stale row versions, and retain normal reconciliation/leg transitions. Hook tests cover both spectator and scoring modes.
- `src/hooks/useRealtime.ts`: Separate receive-only private score channel alongside Postgres Changes; match-player INSERT/UPDATE filters are server-side, DELETE remains client-filtered for Supabase compatibility.
...
- `src/app/new/page.tsx`: Tournament fixtures select the office and its named board; unplayed head-to-head tournament fixtures support explicit manual scoring while retaining office reservations and locked rules. Manual Bengt starts show a red inline warning and a blocking, keyboard-accessible confirmation requiring a fresh acknowledgement; `src/app/new/page.test.tsx` covers cancellation, acknowledgement, board-backed starts and ordinary matches.
